import { Stack, Typography, Button, Alert, Divider, Switch, FormControlLabel, List, ListItem, ListItemText, Chip } from '@mui/material';
    ...
    <Stack spacing={2}>
      ...
      {hasAlerts && (
        <Stack spacing={1}>
          ...
          {lowBattery.map((d) => (
            <Alert key={d.id} severity="error" sx={{ borderRadius: 2, py: 0.4 }}>
          ...
          {longNoSync.map((d) => (
            <Alert key={d.id} severity="warning" sx={{ borderRadius: 2, py: 0.4 }}>

...
function MapViewUpdater({ center, zoom, follow, target }: {
  center: [number, number]; zoom: number; follow: boolean; target: [number, number];
}) {
  const map = useMap();
  useEffect(() => { if (follow) map.setView(target, map.getZoom()); }, [target, follow, map]);
...
export default function TrackingMap({ store }: Props) {
  const {
    isDark,
    devices, geofences, selectedDeviceId,
    deviceViolations, historyState, historyVisible,
    ...
    <>
      ...
      >
        ...
        {geofences.map((gf) => {
          ...
          return (
            <Polygon key={gf.id} positions={gf.coordinates}
              pathOptions={{
                color: gf.color, fillColor: gf.color,
            ...
            >
              <Popup>
                <Box sx={{ minWidth: 200, p: 0.5 }}>
                  <Typography variant="subtitle2" sx={{ fontWeight: 700 }}>{gf.name}</Typography>
                  {gf.address && <Typography variant="caption" display="block" color="text.secondary">{gf.address}</Typography>}
          ...
          return (
            <>
              {gf.coordinates.map((coord, idx) => (
                <Marker key={`${gf.id}-v${idx}`} position={coord} draggable
                  icon={createVertexIcon(gf.color)}
                  eventHandlers={{ dragend: (e) => handleDragVertexEnd(gf.id, idx, e) }}
                />
              ))}
              <Marker key={`${gf.id}-center`} position={getPolygonCentroid(gf.coordinates)} draggable
                icon={createCenterMoveIcon(gf.color)}
                eventHandlers={{ dragstart: handleCenterDragStart, dragend: (e) => handleCenterDragEnd(gf.id, e) }}
              />
            </>
          );
        })()}
        ...
        {devices.map((dev) => {
          ...
          return (
            <Polyline key={`history-${dev.id}`} positions={positions}
              pathOptions={{ color: dev.color, weight: 4, opacity: 0.55 }}
            >
              <Popup>
                <Box sx={{ minWidth: 190, p: 0.5 }}>
                  <Typography variant="subtitle2" sx={{ fontWeight: 700 }}>Lịch sử — {dev.name}</Typography>
                  <Typography variant="caption" display="block">
        ...
        {devices.flatMap((dev) => {
          ...
          if (first !== last)
            markers.push(
              <Marker key={`hs-${dev.id}`} position={[first.lat, first.lng]}
          ...
          markers.push(
            <Marker key={`he-${dev.id}`} position={[last.lat, last.lng]}
        ...
        {devices.map((dev) =>
          dev.pathHistory.length > 1 ? (
            <Polyline key={`trail-${dev.id}`} positions={dev.pathHistory}
              pathOptions={{
                color: deviceViolations[dev.id] ? '#ef4444' : dev.color,
                weight: 2, dashArray: '5,10', opacity: 0.7,
              }}
            />
          ) : null,
        )}
        ...
        {/* Device markers */}
        {devices.map((dev) => (
          <Marker key={dev.id} position={dev.coords}
            icon={createDeviceIcon(dev.angle, !!deviceViolations[dev.id], dev.color, dev.id === selectedDeviceId)}
            eventHandlers={{ click: () => { setSelectedDeviceId(dev.id); setActiveTab(0); } }}
          >
            <Popup>
              <Box sx={{ minWidth: 210, p: 0.5 }}>
                <Typography variant="subtitle2" sx={{ fontWeight: 800, mb: 0.5 }}>📍 {dev.name}</Typography>
                {dev.subject && (
                  <Box sx={{ bgcolor: 'rgba(0,0,0,0.04)', borderRadius: 1.5, p: 1, mb: 1 }}>
                    <Typography variant="body2" sx={{ fontWeight: 700 }}>{dev.subject.fullName}</Typography>
                    <Typography variant="caption" display="block" sx={{ color: '#ef4444', fontWeight: 600 }}>{dev.subject.crime}</Typography>
                    <Typography variant="caption" display="block">Mãn hạn: {dev.subject.releaseDate || '—'}</Typography>
                  </Box>
                )}
                <Typography variant="caption" display="block">IMEI: {dev.uniqueId}</Typography>
                <Typography variant="caption" display="block">
                  Tọa độ: {dev.coords[0].toFixed(5)}, {dev.coords[1].toFixed(5)}
                </Typography>
                <Typography variant="caption" display="block">Pin: {dev.status.battery}% · GPS: {dev.status.gpsAccuracy}m</Typography>
                <Typography variant="caption" display="block"

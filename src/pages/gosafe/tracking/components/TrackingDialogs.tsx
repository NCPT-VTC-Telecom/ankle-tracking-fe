import {
  ...
  Divider, Typography, Avatar, Chip, Box, List, ListItem,
  ...
  return (
    <Stack spacing={2.5}>
      ...
      <Stack direction="row" spacing={1} alignItems="center" className="gs-swatch-row">
        <Typography variant="body2" color="text.secondary">Màu marker:</Typography>
      ...
      {f.type === 'Person' && (
        <>
          <Divider>
            <Typography variant="caption" color="text.secondary" sx={{ px: 1 }}>Thông tin phạm nhân</Typography>
          </Divider>
          <Stack direction="row" spacing={2}>
          ...
          <TextField label="Bản án / Hình phạt" fullWidth value={f.subjectSentence} onChange={(e) => set({ subjectSentence: e.target.value })} />
          <Stack direction="row" spacing={2}>
            <TextField label="Ngày bắt đầu" fullWidth type="date" InputLabelProps={{ shrink: true }}
              value={f.subjectStartDate} onChange={(e) => set({ subjectStartDate: e.target.value })} />
            <TextField label="Ngày kết thúc" fullWidth type="date" InputLabelProps={{ shrink: true }}
              value={f.subjectReleaseDate} onChange={(e) => set({ subjectReleaseDate: e.target.value })} />
          </Stack>
          <TextField label="Ghi chú" fullWidth multiline rows={2} value={f.subjectNotes}
...
function GfColorRow({ value, onChange }: { value: string; onChange: (c: string) => void }) {
  return (
    <Stack direction="row" spacing={1} alignItems="center" className="gs-swatch-row">
      <Typography variant="body2" color="text.secondary">Màu:</Typography>
      {GF_COLORS.map((c) => (
        <Box key={c}
...
export default function TrackingDialogs({ store }: Props) {
  const {
    isDark,
    devices, geofences,
    addDeviceOpen, setAddDeviceOpen, addDeviceForm,
    ...
    editGfId, setEditGfId, editGfForm, setEditGfForm,
    addGfOpen, setAddGfOpen, addGfForm, setAddGfForm,
    handleAddDevice, handleSaveEditDevice, handleRemoveDevice,
    handleAssignDeviceToGeofence, handleSaveGfInfo, handleAddGeofence,
  } = store;

  const paperSx = { borderRadius: 4, bgcolor: isDark ? '#0f172a' : '#fff', backgroundImage: 'none' };
  ...
  return (
    <>
      ...
      <Dialog open={addDeviceOpen} onClose={() => setAddDeviceOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: paperSx }}>
        ...
        <DialogContent sx={{ pt: 2 }}><DeviceFormContent store={store} /></DialogContent>
      ...
      {/* ── Remove Confirm ── */}
      <Dialog open={!!removeConfirmId} onClose={() => setRemoveConfirmId(null)} maxWidth="xs" fullWidth PaperProps={{ sx: { ...paperSx, borderRadius: 3 } }}>
        ...
        <DialogActions sx={{ px: 3, pb: 3, gap: 1 }}>
          <Button onClick={() => setRemoveConfirmId(null)} sx={{ borderRadius: 2.5, fontWeight: 600, py: 1 }}>Hủy</Button>
          <Button variant="contained" color="error" onClick={() => handleRemoveDevice(removeConfirmId!)}
            sx={{ borderRadius: 2.5, fontWeight: 700, py: 1, px: 3 }}>
            Xoá
          </Button>
        </DialogActions>
            ...
            <DialogContent>
              <Stack spacing={2}>
                <Stack direction="row" spacing={2} alignItems="center">
                  <Avatar sx={{ width: 56, height: 56, bgcolor: dev.color, fontSize: '1.4rem', fontWeight: 800 }}>
                    {sub.fullName.split(' ').slice(-1)[0]?.charAt(0) ?? '?'}
                  </Avatar>
                  <Box>
                    <Typography variant="h6" sx={{ fontWeight: 800, lineHeight: 1.2 }}>{sub.fullName}</Typography>
                    <Typography variant="caption" color="text.secondary">CCCD: {sub.idNumber || '—'}</Typography>
                  </Box>
                </Stack>
                <Divider />
                {[
                  ['Thiết bị', dev.name], ['IMEI', dev.uniqueId],
                  ['Tội danh', sub.crime], ['Bản án', sub.sentence],
                  ['Ngày bắt đầu', sub.startDate || '—'], ['Ngày mãn hạn', sub.releaseDate || '—'],
                ].map(([label, value]) => (
                ...
                {sub.notes && (
                  <Box sx={{ bgcolor: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.03)', borderRadius: 2, p: 1.5 }}>
                    <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 700, display: 'block', mb: 0.5 }}>Ghi chú:</Typography>
            ...
            <DialogContent sx={{ pt: 1.5 }}>
              ...
              <List disablePadding>
                {devices.map((dev) => {
                  ...
                  const other = !isAssigned && dev.assignedGeofenceId
                    ? geofences.find((g) => g.id === dev.assignedGeofenceId)?.name : null;
                  return (
                    <ListItem key={dev.id} disableGutters sx={{ py: 0.5 }}>
                      ...
                      <Avatar sx={{ width: 32, height: 32, bgcolor: dev.color, fontSize: '0.75rem', fontWeight: 800, mr: 1.5, flexShrink: 0 }}>
                        {dev.name.slice(-3)}
                      </Avatar>
                      <ListItemText
                        primary={<Typography variant="body2" sx={{ fontWeight: 700 }}>{dev.name}</Typography>}
                        secondary={
      ...
      <Dialog open={!!editGfId} onClose={() => setEditGfId(null)} maxWidth="xs" fullWidth PaperProps={{ sx: paperSx }}>
        ...
        <DialogContent>
          <Stack spacing={2.5} sx={{ mt: 1 }}>
            <TextField label="Tên vùng *" fullWidth value={editGfForm.name}
              onChange={(e) => setEditGfForm((f) => ({ ...f, name: e.target.value }))} />
            <TextField label="Địa chỉ" fullWidth value={editGfForm.address}
      ...
      <Dialog open={addGfOpen} onClose={() => setAddGfOpen(false)} maxWidth="xs" fullWidth PaperProps={{ sx: paperSx }}>
        ...
        <DialogContent>
          <Stack spacing={2.5} sx={{ mt: 1 }}>
            <TextField label="Tên vùng *" fullWidth value={addGfForm.name}
              onChange={(e) => setAddGfForm((f) => ({ ...f, name: e.target.value }))} />
            <TextField label="Địa chỉ" fullWidth value={addGfForm.address}
        ...
        <DialogActions sx={{ px: 3, pb: 3, gap: 1 }}>
          ...
          <Button variant="contained" onClick={handleAddGeofence} disabled={!addGfForm.name}
            sx={{ borderRadius: 2.5, fontWeight: 700, py: 1, px: 3 }}>Thêm</Button>
        </DialogActions>
      </Dialog>
    </>
  );

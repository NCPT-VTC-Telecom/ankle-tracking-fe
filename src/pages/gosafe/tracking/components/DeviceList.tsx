import {
  ...
  Avatar, Tooltip, Chip, Divider, Button, FormControl,
  InputLabel, Select, MenuItem, Switch, FormControlLabel,
} from '@mui/material';
import { Add, Edit, Trash, DocumentText, Play, Pause, Refresh2 } from 'iconsax-react';
...
export default function DeviceList({ store }: Props) {
  const {
    isDark, primaryColor,
    filteredDevices, selectedDeviceId, setSelectedDeviceId,
    deviceViolations, geofences, syncMinutesMap,
    simSpeed, setSimSpeed, followDevice, setFollowDevice,
    openEditDevice, setRemoveConfirmId, setSubjectDetailId, setAddDeviceOpen,
  ...
  return (
    <Stack spacing={2}>
      ...
      {filteredDevices.map((dev) => {
        ...
        return (
          <Card key={dev.id} variant="outlined"
            ...
            sx={{
              ...
              bgcolor: isViolating
                ...
                : isSelected ? isDark ? `${dev.color}11` : `${dev.color}08` : 'transparent',
              '&:hover': { borderColor: dev.color, transform: 'translateY(-1px)', boxShadow: `0 4px 16px ${dev.color}22` },
            }}>
            <CardContent sx={{ p: 2.5, '&:last-child': { pb: 2.5 } }}>
              ...
              <Stack direction="row" justifyContent="space-between" alignItems="center" mb={1.5}>
                ...
                <Stack direction="row" spacing={0.5}>
                  <Tooltip title="Sửa thiết bị">
                    <IconButton size="small" onClick={(e) => { e.stopPropagation(); openEditDevice(dev); }} sx={{ color: primaryColor }}>
                      <Edit size="15" />
                    </IconButton>
                  </Tooltip>
                  <Tooltip title="Xoá thiết bị">
                    <IconButton size="small" onClick={(e) => { e.stopPropagation(); setRemoveConfirmId(dev.id); }} sx={{ color: '#ef4444' }}>
                      <Trash size="15" />
                    </IconButton>
                  </Tooltip>
                </Stack>
              ...
              {/* Subject banner */}
              {dev.subject && (
                <Box sx={{ bgcolor: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.03)', borderRadius: 2, p: 1.5, mb: 1.5 }}>
                  <Stack direction="row" spacing={1.5} alignItems="flex-start">
                    <Avatar sx={{ width: 36, height: 36, bgcolor: dev.color, fontSize: '0.85rem', fontWeight: 700, flexShrink: 0 }}>
                      {dev.subject.fullName.split(' ').slice(-1)[0]?.charAt(0) ?? '?'}
                    </Avatar>
                    <Box sx={{ minWidth: 0 }}>
                      <Typography variant="body2" sx={{ fontWeight: 700, lineHeight: 1.3 }}>{dev.subject.fullName}</Typography>
                      <Typography variant="caption" color="text.secondary" sx={{ display: 'block' }}>
                        CCCD: {dev.subject.idNumber || '—'}
                      </Typography>
                      <Typography variant="caption" sx={{ color: '#ef4444', fontWeight: 600 }}>{dev.subject.crime}</Typography>
                    </Box>
                    <Tooltip title="Xem hồ sơ chi tiết">
                      <IconButton size="small" onClick={(e) => { e.stopPropagation(); setSubjectDetailId(dev.id); }}
              ...
              {isSelected ? (
                <Stack spacing={0} sx={{ mt: 0.5 }}>
                  ...
                  <Stack direction="row" justifyContent="space-between" alignItems="center" py={0.8}>
                    ...
                    <Box className="gs-battery"
                      style={{ '--battery-color': getBatteryColor(dev.status.battery) } as React.CSSProperties}>
                      <Box className="gs-battery__shell"><Box className="gs-battery__fill" style={{ width: `${dev.status.battery}%` }} /></Box>
                  ...
                  {/* Signal */}
                  <Stack direction="row" justifyContent="space-between" alignItems="center" py={0.8}>
                    <Typography variant="body2" color="text.secondary">GSM:</Typography>
                    <Stack direction="row" alignItems="center" spacing={0.5}>
                      <Box className="gs-signal">
                        {[1, 2, 3, 4].map((b) => (
                          <Box key={b} className="gs-signal__bar" style={{
                            height: b * 3.5,
                            backgroundColor: b <= dev.status.signalStrength
                              ? '#22c55e' : isDark ? 'rgba(255,255,255,0.18)' : 'rgba(0,0,0,0.14)',
                          }} />
                        ))}
                      </Box>
                      <Typography variant="caption" sx={{ fontWeight: 600, lineHeight: 1 }}>{dev.status.signalStrength}/4</Typography>
                    </Stack>
                  </Stack>
                  ...
                  <Stack direction="row" justifyContent="space-between" py={0.8}>
                    ...
                    <Typography variant="body2" sx={{ fontWeight: 600 }}>
                      {timeAgo(dev.status.lastGpsUpdate)} · {dev.status.gpsAccuracy}m
                    </Typography>
                  </Stack>

                  <Stack direction="row" justifyContent="space-between" py={0.8}>
                    <Typography variant="body2" color="text.secondary">Sync:</Typography>
                    <Typography variant="body2" sx={{ fontWeight: 600, color: (syncMinutesMap[dev.id] ?? 0) > 30 ? '#f59e0b' : 'inherit' }}>
                      {timeAgo(dev.status.lastServerSync)}
                    </Typography>
                  </Stack>

                  <Stack direction="row" justifyContent="space-between" alignItems="center" py={0.8}>
                    <Typography variant="body2" color="text.secondary">Vùng:</Typography>
                    <Chip
                      ...
                      size="small" sx={{ fontWeight: 700, height: 22, fontSize: '0.7rem' }}
                    />
                  </Stack>
                  ...
                  <Stack direction="row" justifyContent="space-between" py={0.8}>
                    ...
                    <Typography variant="body2" sx={{ fontWeight: 600 }}>
                      {dev.isSimulating ? `${(simSpeed * 12).toFixed(1)} km/h` : '0 km/h'}
                    </Typography>
                  </Stack>
                  ...
                  <Stack direction="row" spacing={1.5} mt={1.5}>
                    <Button variant="contained" fullWidth color={dev.isSimulating ? 'warning' : 'primary'}
                      startIcon={dev.isSimulating ? <Pause size="15" /> : <Play size="15" />}
                      onClick={(e) => { e.stopPropagation(); handleToggleSimulate(dev.id); }}
                      sx={{ borderRadius: 2.5, py: 0.9, fontWeight: 700 }}>
                      {dev.isSimulating ? 'Dừng' : 'Chạy'}
                    </Button>
                    <Button variant="outlined" fullWidth startIcon={<Refresh2 size="15" />}
                      onClick={(e) => { e.stopPropagation(); handleResetDevice(dev.id); }}
                      sx={{ borderRadius: 2.5, py: 0.9, fontWeight: 700 }}>
                      Đặt lại
                    </Button>
                  </Stack>
                  ...
                  <FormControl fullWidth size="small" sx={{ mt: 1.5 }}>
                    <InputLabel>Tốc độ mô phỏng</InputLabel>
                    <Select value={simSpeed} label="Tốc độ mô phỏng"
                      onChange={(e) => setSimSpeed(Number(e.target.value))} sx={{ borderRadius: 2.5 }}>
                      <MenuItem value={1}>1× — 1.0 giây/bước</MenuItem>
                      <MenuItem value={2}>2× — 0.5 giây/bước</MenuItem>
                      <MenuItem value={5}>5× — 0.2 giây/bước</MenuItem>
                    </Select>
                  </FormControl>

...
const ContactSection = ({ isDark, primaryColor, secondaryColor }: ContactSectionProps) => {
  ...
  const [formData, setFormData] = useState({
    fullname: '', senderEmail: '', phoneNumber: '',
    companyName: '', subject: '', content: ''
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      enqueueSnackbar(intl.formatMessage({ id: 'gosafe-contact-submit-success', defaultMessage: 'Gửi yêu cầu thành công!' }), { variant: 'success' });
      setFormData({ fullname: '', senderEmail: '', phoneNumber: '', companyName: '', subject: '', content: '' });
    } catch {
      enqueueSnackbar(intl.formatMessage({ id: 'gosafe-contact-submit-error', defaultMessage: 'Có lỗi xảy ra.' }), { variant: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const inputSx = {
    '& .MuiOutlinedInput-root': {
      ...
      '& fieldset': { borderColor: isDark ? alpha('#fff', 0.1) : alpha('#000', 0.1), borderWidth: '1px' },
  ...
  const contactItems = [
    {
      ...
      titleKey: 'gosafe-contact-info-address', titleDefault: 'Địa chỉ',
    ...
    },
    {
      ...
      titleKey: 'gosafe-contact-info-phone', titleDefault: 'Số điện thoại',
      contentKey: 'gosafe-contact-info-phone-value', contentDefault: '0901 418 053',
      isLink: true, href: 'tel:0901418053'
    },
    {
      icon: <Sms size={24} variant="Bold" />,
      titleKey: 'gosafe-contact-info-email', titleDefault: 'Email',
      contentKey: 'gosafe-contact-info-email-value',
      contentDefault: 'nam.nguyen-hoang@vtctelecom.com.vn',
      isLink: true, href: 'mailto:nam.nguyen-hoang@vtctelecom.com.vn'
    }
  ];
        ...
        >
          <Grid container>
            ...
            <Grid item xs={12} md={5} sx={{ borderRight: { md: `1px solid ${borderColor}` } }}>
              <Stack sx={{ height: '100%', flexDirection: 'column', justifyContent: 'space-between', p: { xs: 3, md: 6 } }}>
                <Box>
                  ...
                  <Typography variant="body1" sx={{ color: theme.palette.text.secondary, lineHeight: 1.7, fontSize: '1.05rem' }}>
                    <FormattedMessage id="gosafe-contact-desc" defaultMessage="Hãy để lại thông tin, đội ngũ chuyên gia của VTC Telecom sẽ liên hệ giải đáp mọi thắc mắc của bạn." />
                  </Typography>
                </Box>
                ...
                <Stack spacing={3} sx={{ mt: 2 }}>
                  {contactItems.map((item, i) => (
                    <Stack key={i} direction="row" spacing={2.5} alignItems="flex-start">
                      ...
                      <Box className="gs-contact-icon" sx={{ bgcolor: alpha(primaryColor, 0.1), color: primaryColor }}>
                        {item.icon}
                      </Box>
                      <Box>
                        ...
                        {item.isLink ? (
                          ...
                          <Typography
                            ...
                            sx={{ color: theme.palette.text.secondary, '&:hover': { color: primaryColor } }}
                          >
                            {item.contentDefault}
                          </Typography>
                        ) : (
            ...
            <Grid item xs={12} md={7} sx={{ p: { xs: 3, md: 6 } }}>
              <Box component="form" onSubmit={handleSubmit}>
                <Grid container spacing={2.5}>
                  {[
                    { label: 'gosafe-contact-form-fullname', def: 'Họ và tên', ph: 'gosafe-contact-form-fullname-placeholder', phDef: 'Nguyễn Văn A', name: 'fullname', val: formData.fullname, req: true },
                    { label: 'gosafe-contact-form-email', def: 'Email', ph: 'gosafe-contact-form-email-placeholder', phDef: 'email@domain.com', name: 'senderEmail', val: formData.senderEmail, type: 'email', req: true },
                    { label: 'gosafe-contact-form-phone', def: 'Số điện thoại', ph: 'gosafe-contact-form-phone-placeholder', phDef: '0901 418 053', name: 'phoneNumber', val: formData.phoneNumber, req: true },
                    { label: 'gosafe-contact-form-company', def: 'Công ty', ph: 'gosafe-contact-form-company-placeholder', phDef: 'Tên doanh nghiệp', name: 'companyName', val: formData.companyName }
                  ].map((f) => (
                  ...
                  <Grid item xs={12}>
                    ...
                    <TextField
                      fullWidth
                      placeholder={intl.formatMessage({ id: 'gosafe-contact-form-subject-placeholder', defaultMessage: 'Vấn đề cần hỗ trợ / Yêu cầu báo giá' })}
                      name="subject"
                  ...
                  <Grid item xs={12}>
                    ...
                    <TextField
                      fullWidth
                      placeholder={intl.formatMessage({ id: 'gosafe-contact-form-content-placeholder', defaultMessage: 'Chi tiết nội dung bạn cần tư vấn...' })}
                      name="content"
                      multiline
                      rows={4}

...
interface GlobalFilterProps extends OutlinedInputProps {
  /** Hàm callback khi người dùng thay đổi giá trị search */
  searchFilter: (value: string) => void;
}

export function GlobalFilter({ searchFilter, ...other }: GlobalFilterProps) {
  const intl = useIntl();
  ...
  return (
    <OutlinedInput
      ...
      placeholder={intl.formatMessage({ id: 'search-record' })}
